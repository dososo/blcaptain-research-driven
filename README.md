...
1. **它最坑你的时候,恰恰是它最自信的时候。** 你让它查「这个 API 现在还能用吗」「这两个版本兼不兼容」,它斩钉截铁给你个答案——你照做,结果是错的、或者早过时了。它不是「不知道」(不知道它会去查),是「以为自己知道」:越笃定,越不去核。
...
- **更省。** 省的不是搜索那几个 token——是省掉「你信了错答案 → 照着做 → 发现不对 → 推倒重来」这条最贵的路。它还会按重要性分配精力:常识直接跳过、只对关键结论深挖、够了就停,不瞎查一通。**一次做对,才是真的省。**
